...
![Team with a visible orchestrator and direct context](docs/images/team.png)

![Solo beside the project editor](docs/images/studio-solo.png)

![Roundtable setup](docs/images/roundtable.png)

![Searchable commands](docs/images/command-palette.png)

## Follow activity across devices

Open **Activity** in the workspace header, the View menu or the command palette. This Mac's open projects appear immediately. Choose **Add a Mac** to follow another computer. On that computer, enable **Agent settings → Remote** and choose **Pair a Device**; paste its code into Activity. Repeat on each device where you want the combined view. Up to eight Macs can be paired per viewer, and each host accepts up to eight simultaneous viewers.

**Orrery Remote 1.3** brings the same dashboard to iPhone and iPad. Build it from `Remote/OrreryRemote` with Xcode and your own signing account; scan or paste each Mac's pairing code. The public release does not include an App Store or TestFlight download.

Activity covers work started in Orrery: Solo conversations, Team tasks, Roundtable and native CLI session status. Search projects and agents, filter working items or decisions, and open a conversation to read, send, approve or stop. Viewing on another device does not switch the Mac's screen. CLI output and provider-owned prompts remain on the host Mac. Chats started in other apps are not imported.

Keep host Macs awake with Orrery open. Use the same network, a private VPN, or deploy `Remote/OrreryRelay` to your own Cloudflare account. Configure its URL on each host **before pairing**. Existing relay owners must deploy Relay 2 for simultaneous viewers; older relays support one viewer. No central Orrery account or hosted service is provided. Relay hosting may incur charges on your own account.

Each device's pairing key and saved relay credentials stay in Keychain. Transcripts and drafts remain in memory on viewers and are cleared when the app quits. Offline snapshots show their last update instead of live progress. Remove a device on the host Mac to revoke it; forgetting a Mac on a viewer removes only that viewer's saved connection. Pairing codes expire after five minutes and are single-use.
...
```sh
cd Remote/OrreryRelay && npm ci && npm test && cd ../..
swift build
